...
# Views for loading data from csv file to the template
class ShowCsvInTemplate(View):
    template_name = "onion_leafs/show_table_csv.html"
    ...
    def generateGraph(graphData, title):
        ...
        graphData["high"] = graphData["high"].str.replace(",", "").astype(float)
        graphData["low"] = graphData["low"].str.replace(",", "").astype(float)
        graphData["open"] = graphData["open"].str.replace(",", "").astype(float)
        graphData["close"] = graphData["close"].str.replace(",", "").astype(float)
        graphData["volume"] = graphData["volume"].str.replace(",", "").astype(int)
        ...
        # Export as HTML
        fig.write_html("onion/templates/onion_leafs/plot_csv.html")

    def get(self, request, *args, **kwargs):
        trade_name = request.GET.get("trade")
        read_csv = pd.read_csv("onion/data/stock_market_data.csv")
        graphData = pd.read_csv("onion/data/stock_market_data.csv")

        # convert csv to list of dictionary
        ...
        mean_values = df.groupby("trade_code").mean()
